...
public class MediaPlayerCtrl : MonoBehaviour
{
    ...
    public static bool is3dUD = true; // 是不是上下3D视频
    
 
 //   public Slider m_VolumeSlider; //音量滑动条
  
    private int currentLevel = 0;//当前的码率层级
    private int toBeChangeLevel; //待切换的码率层级

    private int recordChangeLevelTime = 0; //多码率切换记录切换时间
    private bool needSeekToRecordChangeLevelTime = false; //多码率切换，是否需要seek 进度
	...
	public MEDIAPLAYER_STATE m_CurrentState; //修改为public
	private int m_iCurrentSeekPosition;
	public float m_fVolume = 0.6f; //修改为public
    ...
    //多码率切换
    public void ChangeLevelWithIndex(int index)
    {
        toBeChangeLevel = index;

        Debug.Log("ChangeLevelWithIndex!!!!!!!!!!!");
        if (Application.platform == RuntimePlatform.Android)
        {
            AndroidAPI.ChangeLevelForUnityTV189(index);
        }
        else
        {
            string path = "http://192.168.0.110/pv/mp4/a0000011-2015CJHDjuesai-2k.mp4";
            if (index == 0)
            {
                path = "http://192.168.0.110/pv/mp4/a0000011-2015CJHDjuesai-2k.mp4";
            }
            else if (index == 1)
            {
                path = "http://192.168.0.110/pv/mp4/nature.mp4";
            }
            else if (index == 2)
            {
                path = "http://cache.utovr.com/f2ddb91cd6a346c59ef170028a7300c7/L4_flxrrcts3bmumihl.m3u8";
            }
            else if (index == 3)
            {
                path = "http://192.168.0.110/pv/mp4/Samsung_Gear_360.mp4";
            }
            else if (index == 4)
            {
               // path = "http://192.168.0.110/pv/mp4/L2_8f0tz7r1f0j952zd.mp4";
               path = "";
            }

            ChangeLevelWithPath(path);
        }
    }

    public void ChangeLevelWithPath(string path)
    {
        Debug.Log("ChangeLevelWithPath!!!!!!!!!!!" + path);
        if (!string.IsNullOrEmpty(path))
        {
            //改变level
            currentLevel = toBeChangeLevel;

            recordChangeLevelTime = m_iCurrentSeekPosition;
            this.UnLoad();

            m_iCurrentSeekPosition = recordChangeLevelTime;
            needSeekToRecordChangeLevelTime = true;
            this.Load(path);
        }
    }
	...
	void Update()
	{
        ...
        //同步更新音量
//#if UNITY_ANDROID && !UNITY_EDITOR
        m_fVolume = Call_GetVolume();
        //if (m_VolumeSlider != null && m_VolumeSlider.value != m_fVolume)
        //    m_VolumeSlider.value = m_fVolume;
//#endif
	...
	Call_Load(strName,0);
		...
		if (m_CurrentState == MEDIAPLAYER_STATE.PLAYING || m_CurrentState == MEDIAPLAYER_STATE.PAUSED)
		{
			if (m_bCheckFBO == false) {
                ...
                ///////////以下新添加的代码///////////
                if (needSeekToRecordChangeLevelTime)
                {
                    needSeekToRecordChangeLevelTime = false;
                    SeekTo(recordChangeLevelTime);
                }
                ///////////以上新添加的代码///////////
		...
		if (m_CurrentState != Call_GetStatus())
		{
			...
			else if (m_CurrentState == MEDIAPLAYER_STATE.END)
			{
                ...
                else
                {
                    ///////////以下新添加的代码///////////
                    //隐藏 视频工具条面板
                    //m_VideoControl.SetActive(false);
                    ////隐藏 工具条面板
	...
	private void Call_SetVolume(float fVolume)
	{
        ...
        AndroidAPI.setVolume(fVolume);
	}
    private float Call_GetVolume()
    {
        return AndroidAPI.getVolume();
	...
	public  void Call_Play(int iSeek)
	{
